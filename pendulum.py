#Este archivo resuelve la ecuacion diferencial segunda y''=-Asen(y) usando Runge Kutta de 4 orden, un parametro y 2 condiciones iniciales en el siguiente orden: 1) parametro A=2 que acompana el sen(y) en la eq diferencial. 2) Y(0)=0. 3) Y'(0)=0
...
y_1[0] = B
y_2[0] = C    
file = open('Solucion.dat','w')
for i in range(1,n_points):
    x[i],y_1[i],y_2[i] = RungeKuttaFourthOrderStep(x[i-1], y_1[i-1], y_2[i-1])
    file.write(str(x[i])+"  "+str(y_1[i])+"\n")
file.close()

print ("el archivo ha corrido con exito, usando las condiciones iniciales A="+str(A)+"Y(0)="+str(B)+"Y'(0)"+str(C)+"\n")
